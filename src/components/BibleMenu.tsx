...
import type { Book, Translation } from "../App";
...
const BibleMenu: React.FC<BibleMenuProps> = ({
	...
	const [_verse, setVerse] = React.useState<number | null>(selectedVerse);
				...
				<div>
					...
					>
						<ArrowBackIosNewOutlinedIcon
							onClick={handleBack}
							sx={{ alignSelf: "center", marginRight: ".5rem" }}
						/>
						<h2>
							{
								translations?.find(
									(translation) => translation?.language === language,
								)?.bible[language === "en" ? book : book - 39]?.title
							}{" "}
							...
							{Array.from(
								{
									length:
										translations?.find(
											(translation) => translation?.language === language,
										)?.bible[language === "en" ? book : book - 39]?.chapters ??
										0,
								},
								(_, i) => i + 1,
							).map((chapter) => (
